export interface BlockNode { id: string; label: string; parentId?: string; span: number; }
export interface BlockComposite { columns?: number; id: string; parentId?: string; span: number; }
...
interface CompositeRecord extends BlockComposite { close: Line; columnsLine?: Line; line: Line; }
...
export function getBlockDiagramSnapshot(source: string): BlockDiagramSnapshot { const parsed = requireBlock(source); return { ...(parsed.columns ? { columns: parsed.columns } : {}), nodes: parsed.nodes.map(({ id, label, parentId, span }) => ({ id, label, span, ...(parentId ? { parentId } : {}) })), composites: parsed.composites.map(({ id, parentId, span, columns }) => ({ id, span, ...(parentId ? { parentId } : {}), ...(columns ? { columns } : {}) })), links: parsed.links.map(({ from, to }) => ({ from, to })) }; }
...
export function addBlockNode(source: string, node: Partial<BlockNode> = {}): string {
  ...
  if (!source.trim()) return `block-beta\n  ${statement}`;
  if (!node.parentId) return append(source, `  ${statement}`);
  const parent = findComposite(parsed!, node.parentId); return `${source.slice(0, parent.close.start)}${indent(parent.line)}  ${statement}${lineEnding(source)}${source.slice(parent.close.start)}`;
}
...
export function moveBlockNode(source: string, id: string, parentId: string | null): string { const parsed = requireBlock(source); const node = findNode(parsed, id); if ((node.parentId ?? null) === parentId) return source; const without = deleteLines(source, [node.line]); if (!parentId) return append(without, `  ${formatNode(node)}`); const parent = findComposite(requireBlock(without), parentId); return `${without.slice(0, parent.close.start)}${indent(parent.line)}  ${formatNode(node)}${lineEnding(without)}${without.slice(parent.close.start)}`; }
export function moveBlockComposite(source: string, id: string, parentId: string | null): string {
  const parsed = requireBlock(source); const composite = findComposite(parsed, id);
  if ((composite.parentId ?? null) === parentId) return source;
  if (parentId === id || (parentId && isCompositeDescendant(parsed, parentId, id))) throw new Error('A block composite cannot contain itself.');
  const fragment = source.slice(composite.line.start, composite.close.end);
  const without = deleteLines(source, [{ start: composite.line.start, end: composite.close.end, raw: '', text: '' }]);
...
export function addBlockComposite(source: string, composite: Partial<BlockComposite> = {}): string {
  const parsed = requireBlock(source);
  const id = uniqueId(normalizeId(composite.id ?? 'Group'), allIds(parsed));
  const span = normalizeSpan(composite.span ?? 1);
  const columns = composite.columns === undefined ? undefined : normalizeColumns(composite.columns);
  const ending = lineEnding(source);
  if (!composite.parentId) return append(source, formatComposite(id, span, columns, '  ', ending));
  const parent = findComposite(parsed, composite.parentId);
  const childIndent = `${indent(parent.line)}  `;
  return `${source.slice(0, parent.close.start)}${formatComposite(id, span, columns, childIndent, ending)}${ending}${source.slice(parent.close.start)}`;
}
export function editBlockComposite(source: string, id: string, patch: Partial<BlockComposite>): string { const parsed = requireBlock(source); const current = findComposite(parsed, id); if (patch.columns !== undefined && !current.columnsLine) { const columns = normalizeColumns(patch.columns); const seeded = `${source.slice(0, current.close.start)}${indent(current.line)}  columns ${columns}${lineEnding(source)}${source.slice(current.close.start)}`; const { columns: _columns, ...remaining } = patch; return editBlockComposite(seeded, id, remaining); } const nextId = patch.id === undefined ? id : normalizeId(patch.id); const span = patch.span === undefined ? current.span : normalizeSpan(patch.span); const columns = patch.columns === undefined ? current.columns : normalizeColumns(patch.columns); if (nextId !== id && allIds(parsed).includes(nextId)) throw new Error(`A block item named ${nextId} already exists.`); return replaceValues(source, [{ line: current.line, value: `${indent(current.line)}block:${nextId}${span === 1 ? '' : `:${span}`}` }, ...(patch.columns !== undefined && current.columnsLine ? [{ line: current.columnsLine, value: `${indent(current.columnsLine)}columns ${columns}` }] : []), ...parsed.links.filter((link) => nextId !== id && (link.from === id || link.to === id)).map((link) => ({ line: link.line, value: `${indent(link.line)}${formatLink({ from: link.from === id ? nextId : link.from, to: link.to === id ? nextId : link.to })}` }))]); }
export function deleteBlockComposite(source: string, id: string): string { const parsed = requireBlock(source); const composite = findComposite(parsed, id); const descendants = new Set<string>([id]); let changed = true; while (changed) { changed = false; for (const item of [...parsed.nodes, ...parsed.composites]) if (item.parentId && descendants.has(item.parentId) && !descendants.has(item.id)) { descendants.add(item.id); changed = true; } } return deleteLines(source, [{ start: composite.line.start, end: composite.close.end, raw: '', text: '' }, ...parsed.links.filter((link) => descendants.has(link.from) || descendants.has(link.to)).map((link) => link.line)]); }
...
function parseBlock(source: string): Parsed | null {
  ...
  const nodes: NodeRecord[] = []; const composites: CompositeRecord[] = []; const links: LinkRecord[] = []; const stack: { columns?: number; columnsLine?: Line; id: string; line: Line; parentId?: string; span: number }[] = []; let rootColumns: number | undefined;
  for (let index = headerIndex + 1; index < lines.length; index += 1) { const line = lines[index]!; if (!line.text.trim() || ignorable(line.text)) continue;
    if (/^\s*end\s*$/i.test(line.text)) { const open = stack.pop(); if (!open) return null; composites.push({ id: open.id, span: open.span, ...(open.parentId ? { parentId: open.parentId } : {}), ...(open.columns ? { columns: open.columns } : {}), ...(open.columnsLine ? { columnsLine: open.columnsLine } : {}), line: open.line, close: line }); continue; }
    const columns = line.text.match(COLUMNS); if (columns) { if (stack.length) { const active = stack.at(-1)!; if (active.columns) return null; active.columns = Number(columns[1]); active.columnsLine = line; } else { if (rootColumns) return null; rootColumns = Number(columns[1]); } continue; }
    const block = line.text.match(BLOCK); if (block) { const id = block[1]!; if (!isSafeId(id) || allIds({ nodes, composites: composites.concat(stack.map((entry) => ({ ...entry, close: entry.line } as CompositeRecord))) }).includes(id)) return null; stack.push({ id, span: Number(block[2] ?? '1'), parentId: stack.at(-1)?.id, line }); continue; }
    const link = line.text.match(LINK); if (link) { if (stack.length) return null; links.push({ from: link[1]!, to: link[2]!, line }); continue; }
    const node = line.text.match(NODE); if (node) { const id = node[1]!; if (!isSafeId(id) || allIds({ nodes, composites: composites.concat(stack.map((entry) => ({ ...entry, close: entry.line } as CompositeRecord))) }).includes(id)) return null; nodes.push({ id, label: node[2] ?? id, span: Number(node[3] ?? '1'), parentId: stack.at(-1)?.id, line }); continue; }
    return null;
  }
...
function isCompositeDescendant(parsed: Parsed, candidateId: string, ancestorId: string): boolean { let current = parsed.composites.find((composite) => composite.id === candidateId); while (current?.parentId) { if (current.parentId === ancestorId) return true; current = parsed.composites.find((composite) => composite.id === current!.parentId); } return false; }
...
function formatComposite(id: string, span: number, columns: number | undefined, indentation: string, ending: string): string {
  return `${indentation}block:${id}${span === 1 ? '' : `:${span}`}${ending}${columns ? `${indentation}  columns ${columns}${ending}` : ''}${indentation}end`;
}
