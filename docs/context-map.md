...
| Render/navigation | Mermaid parser results map through the version-pinned capability catalog; a local per-diagram registry holds derived SVG, family/mode, and parse errors. The family adapter decides whether source is representable for scoped controls while Y.Text remains canonical. | `session-workspace.tsx`, `diagram-capabilities.ts`, `diagram-preview.ts`, `diagram-canvas.tsx`, `svg-hit-map.ts` | `diagram-capabilities.test.ts`, `diagram-preview.test.ts`, `pnpm test:e2e-sequence`, `/tmp/arielcharts-sequence.png`, `/tmp/arielcharts-sequence-isolation.png` |
...
| Mermaid family capability catalog | Keep the version-pinned parser-family matrix and source-model adapter browser-derived | The catalog collapses Mermaid aliases, renderer variants, and Railroad grammar variants to 30 visual families. It exposes canvas, semantic-form, source-only, and unavailable-plugin modes without adding Yjs type metadata; adapters fail closed when source cannot safely support a semantic operation. |
