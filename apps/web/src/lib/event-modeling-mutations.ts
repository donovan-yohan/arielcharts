...
export function editEventModelingTimeframe(source: string, index: string, patch: Partial<EventModelingTimeframe> & { index?: string }): string {
  ...
  const replacements = [{ line: current.line, value: `${indent(current.line)}${formatFrame(value)}` }, ...value.index === current.index ? [] : parsed.frames.filter((frame) => frame !== current && frame.links.includes(current.index)).map((frame) => ({ line: frame.line, value: `${indent(frame.line)}${formatFrame({ ...frame, links: frame.links.map((link) => link === current.index ? value.index : link) })}` }))];
...
export function editEventModelingDataBlock(source: string, name: string, patch: Partial<EventModelingDataBlock> & { name?: string }): string {
  ...
  const referenceReplacements = value.name === current.name ? [] : parsed.frames.filter((frame) => frame.dataId === current.name).map((frame) => ({ line: frame.line, value: `${indent(frame.line)}${formatFrame({ ...frame, dataId: value.name })}` }));
  const next = patch.payload === undefined
    ? replaceLines(source, [{ line: current.open, value: formatDataOpen(value, indent(current.open)) }, ...referenceReplacements])
    : replaceRanges(source, [{ end: current.close.end, start: current.open.start, value: `${formatData(value, indent(current.open), lineEnding(source))}${current.close.raw.slice(current.close.text.length)}` }, ...referenceReplacements.map(({ line, value: replacement }) => ({ end: line.end, start: line.start, value: `${replacement}${line.raw.slice(line.text.length)}` }))]);
  return requireEventModeling(next), next;
}
