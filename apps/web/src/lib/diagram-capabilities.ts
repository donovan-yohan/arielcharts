...
const BLOCK_ADAPTER: DiagramSourceModelAdapter = { id: 'block', getOperationResult(source, operation) { return !this.getRepresentability(source).representable ? { supported: false, reason: 'unrepresentable' } : new Set(['add-node', 'edit-node', 'delete-node', 'move-node', 'add-link', 'edit-link', 'delete-link', 'add-composite', 'edit-composite', 'delete-composite', 'move-composite']).has(operation) ? { supported: true } : { supported: false, reason: 'unsupported-operation' }; }, getRepresentability: (source) => isBlockSourceRepresentable(source) ? { representable: true } : { representable: false, reason: 'unsupported-syntax' } };
const C4_ADAPTER: DiagramSourceModelAdapter = { id: 'c4', getOperationResult(source, operation) { return !this.getRepresentability(source).representable ? { supported: false, reason: 'unrepresentable' } : new Set(['add-element', 'edit-element', 'delete-element', 'move-element', 'add-boundary', 'edit-boundary', 'delete-boundary', 'move-boundary', 'add-relationship', 'edit-relationship', 'delete-relationship']).has(operation) ? { supported: true } : { supported: false, reason: 'unsupported-operation' }; }, getRepresentability: (source) => isC4SourceRepresentable(source) ? { representable: true } : { representable: false, reason: 'unsupported-syntax' } };
