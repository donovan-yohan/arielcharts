...
function ArchitectureGroupEditor({ group, groups, onDelete, onSave }: { group: ArchitectureGroup; groups: string[]; onDelete?: (id: string) => void; onSave?: (id: string, patch: Partial<ArchitectureGroup> & { id?: string }) => void }) {
  ...
  return <form aria-label="Architecture group editor" onSubmit={(event) => { event.preventDefault(); onSave?.(group.id, draft); }} style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 5 }}><span>group</span><input aria-label="Architecture group id" onChange={(event) => setDraft((current) => ({ ...current, id: event.target.value }))} value={draft.id} /><input aria-label="Architecture group title" onChange={(event) => setDraft((current) => ({ ...current, title: event.target.value || undefined }))} value={draft.title ?? ''} /><input aria-label="Architecture group icon" onChange={(event) => setDraft((current) => ({ ...current, icon: event.target.value || undefined }))} value={draft.icon ?? ''} /><select aria-label="Architecture group parent" onChange={(event) => setDraft((current) => ({ ...current, parentId: event.target.value || undefined }))} value={draft.parentId ?? ''}><option value="">top level</option>{groups.filter((id) => id !== group.id).map((id) => <option key={id}>{id}</option>)}</select><button type="submit">Save</button><button aria-label="Delete architecture group" onClick={() => onDelete?.(group.id)} type="button">Delete</button></form>;
}
...
function ArchitectureServiceEditor({ groups, onDelete, onSave, service }: { groups: string[]; onDelete?: (id: string) => void; onSave?: (id: string, patch: Partial<ArchitectureService> & { id?: string }) => void; service: ArchitectureService }) {
  ...
  return <form aria-label="Architecture service editor" onSubmit={(event) => { event.preventDefault(); onSave?.(service.id, draft); }} style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 5 }}><span>service</span><input aria-label="Architecture service id" onChange={(event) => setDraft((current) => ({ ...current, id: event.target.value }))} value={draft.id} /><input aria-label="Architecture service title" onChange={(event) => setDraft((current) => ({ ...current, title: event.target.value || undefined }))} value={draft.title ?? ''} /><input aria-label="Architecture service icon" onChange={(event) => setDraft((current) => ({ ...current, icon: event.target.value || undefined }))} value={draft.icon ?? ''} /><select aria-label="Architecture service parent" onChange={(event) => setDraft((current) => ({ ...current, parentId: event.target.value || undefined }))} value={draft.parentId ?? ''}><option value="">top level</option>{groups.map((id) => <option key={id}>{id}</option>)}</select><button type="submit">Save</button><button aria-label="Delete architecture service" onClick={() => onDelete?.(service.id)} type="button">Delete</button></form>;
}
...
function ArchitectureJunctionEditor({ groups, junction, onDelete, onSave }: { groups: string[]; junction: ArchitectureJunction; onDelete?: (id: string) => void; onSave?: (id: string, patch: Partial<ArchitectureJunction> & { id?: string }) => void }) {
  ...
  return <form aria-label="Architecture junction editor" onSubmit={(event) => { event.preventDefault(); onSave?.(junction.id, draft); }} style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 5 }}><span>junction</span><input aria-label="Architecture junction id" onChange={(event) => setDraft((current) => ({ ...current, id: event.target.value }))} value={draft.id} /><select aria-label="Architecture junction parent" onChange={(event) => setDraft((current) => ({ ...current, parentId: event.target.value || undefined }))} value={draft.parentId ?? ''}><option value="">top level</option>{groups.map((id) => <option key={id}>{id}</option>)}</select><button type="submit">Save</button><button aria-label="Delete architecture junction" onClick={() => onDelete?.(junction.id)} type="button">Delete</button></form>;
}
...
function ArchitectureEdgeEditor({ edge, groups, ids, onDelete, onSave }: { edge: ArchitectureEdge; groups: ReadonlySet<string>; ids: string[]; onDelete?: () => void; onSave?: (value: ArchitectureEdge) => void }) {
  ...
  return <form aria-label="Architecture edge editor" onSubmit={(event) => { event.preventDefault(); onSave?.(draft); }} style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 5 }}><select aria-label="Architecture edge source" onChange={(event) => update('from', event.target.value)} value={draft.from}>{ids.map((id) => <option key={id}>{id}</option>)}</select><select aria-label="Architecture edge source port" onChange={(event) => setDraft((current) => ({ ...current, fromPort: event.target.value as ArchitecturePort }))} value={draft.fromPort}>{(['L', 'R', 'T', 'B'] as const).map((port) => <option key={port}>{port}</option>)}</select><label><input checked={draft.fromInto} onChange={(event) => setDraft((current) => ({ ...current, fromInto: event.target.checked }))} type="checkbox" />into source</label><label><input checked={draft.toInto} onChange={(event) => setDraft((current) => ({ ...current, toInto: event.target.checked }))} type="checkbox" />into target</label><select aria-label="Architecture edge target port" onChange={(event) => setDraft((current) => ({ ...current, toPort: event.target.value as ArchitecturePort }))} value={draft.toPort}>{(['L', 'R', 'T', 'B'] as const).map((port) => <option key={port}>{port}</option>)}</select><select aria-label="Architecture edge target" onChange={(event) => update('to', event.target.value)} value={draft.to}>{ids.map((id) => <option key={id}>{id}</option>)}</select><button type="submit">{onDelete ? 'Save' : 'Add edge'}</button>{onDelete ? <button aria-label="Delete architecture edge" onClick={onDelete} type="button">Delete</button> : null}</form>;
}
