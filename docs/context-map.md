...
| Per-diagram overlay scene | Durable/session | Yjs `overlays`; server admission and persistence | Browser owns selection, active tool, drafts, resize handles, and camera. MCP can list/read bounded current object records and make revision-checked object operations only; it never replaces a scene, writes awareness, or changes Mermaid source/layout. Diagram deletion removes its scene and both private histories in one persistence batch. |
