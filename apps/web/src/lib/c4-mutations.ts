...
export interface C4Element { id: string; kind: C4ElementKind; label: string; description?: string; technology?: string; }
export interface C4Boundary { id: string; kind: C4BoundaryKind; label: string; }
...
export function addC4Element(source: string, element: C4Element): string {
  ...
  return append(source, `  ${formatElement({ ...next, id })}`);
}
...
export function addC4Boundary(source: string, boundary: C4Boundary): string { const parsed = requireC4(source); const next = normalizeBoundary(boundary); const id = uniqueId(next.id, parsed.elements.map((entry) => entry.id).concat(parsed.boundaries.map((entry) => entry.id))); return append(source, `  ${next.kind}(${id}, ${quote(next.label)}) {\n  }`); }
...
function parseC4(source: string): ParsedC4 | null {
  ...
  const elements: ElementRecord[] = []; const boundaries: BoundaryRecord[] = []; const relationships: RelationshipRecord[] = []; const stack: { close?: SourceLine; id: string; kind: C4BoundaryKind; label: string; line: SourceLine }[] = [];
  let afterHeader = false;
  for (const line of lines) {
    if (line === headerLine) { afterHeader = true; continue; } if (!afterHeader || !line.text.trim() || ignorable(line.text)) continue;
    if (/^\s*}\s*$/.test(line.text)) { const open = stack.pop(); if (!open) return null; open.close = line; boundaries.push({ id: open.id, kind: open.kind, label: open.label, line: open.line, close: line }); continue; }
    const boundary = line.text.match(BOUNDARY); if (boundary) { const boundaryKind = canonical(BOUNDARY_KINDS, boundary[1]!); if (!boundaryKind || hasId({ elements, boundaries: [...boundaries, ...(stack as BoundaryRecord[])] }, boundary[2]!)) return null; stack.push({ kind: boundaryKind, id: boundary[2]!, label: boundary[3]!, line }); continue; }
    const element = line.text.match(ELEMENT); if (element) { const elementKind = canonical(ELEMENT_KINDS, element[1]!); const id = element[2]!; if (!elementKind || hasId({ elements, boundaries: [...boundaries, ...(stack as BoundaryRecord[])] }, id)) return null; const extras = [element[4], element[5]].filter((value): value is string => value !== undefined); const [technology, description] = elementHasTechnology(elementKind) ? [extras[0], extras[1]] : [undefined, extras[0]]; elements.push({ id, kind: elementKind, label: element[3]!, ...(technology ? { technology } : {}), ...(description ? { description } : {}), line }); continue; }
...
function publicElement(element: ElementRecord): C4Element { return { id: element.id, kind: element.kind, label: element.label, ...(element.technology ? { technology: element.technology } : {}), ...(element.description ? { description: element.description } : {}) }; }
function publicBoundary(boundary: BoundaryRecord): C4Boundary { return { id: boundary.id, kind: boundary.kind, label: boundary.label }; }
