...
| Overlay history | A separate server-derived content revision and private immutable journal snapshots each canonical scene. Browser-cookie routes can list/read/restore it; MCP has no overlay surface. Source restore never writes overlays and overlay restore never writes source/layout. A whole-workspace snapshot is an explicit Mermaid/overlay revision pair, not a timestamp join. | `apps/server/src/lib/persistence.ts`, `session-manager.ts`, `apps/web/src/lib/overlay-history-api.ts`, `packages/shared/src/types.ts` | `session-manager.test.ts`, `index.test.ts`, `overlay-history-api.test.ts` |
| Starter creation | The shared Mermaid family descriptor is the single version-pinned catalog for built-in parser aliases, editing model, stability, help, availability, and minimal starter source. `STARTER_TEMPLATES` preserves the original Blank-plus-six curated public values; distinct immutable primary/chooser collections supply Blank plus the 30 built-in catalog families, while the separately grouped ZenUML plugin starter resolves through `ALL_STARTER_TEMPLATES` like any other accepted browser or MCP ID. The browser keeps semantic adapters and external runtime ownership local. Selected starters resolve to ordinary Mermaid source before the existing browser or revision-checked MCP creation owner runs; template identity is never durable state. | `packages/shared/src/mermaid-diagram-catalog.ts`, `packages/shared/src/starter-templates.ts`, `apps/web/src/lib/diagram-capabilities.ts`, `apps/web/src/components/workspace-template-picker.tsx`, `apps/server/src/lib/mcp.ts`, `apps/server/src/lib/mcp-server.ts` | `starter-templates.test.ts`, `diagram-capabilities.test.ts`, `workspace-template-picker.test.ts`, `mcp.test.ts`, `index.test.ts`, `pnpm test:e2e-workspace-ux` |
...
| Render/navigation | Mermaid parser results map through the version-pinned capability catalog; a local per-diagram registry holds derived SVG, family/mode, and parse errors. Built-ins use the already-loaded Mermaid runtime. A ZenUML top-level source alone enters `zenuml-runtime.ts`, whose process-level promise dynamically imports the pinned bundled plugin, registers it once, and retains either ready or load-failed state; no CDN or durable availability state exists. The family adapter decides whether source is representable for scoped controls while Y.Text remains canonical and the last accepted preview survives load/render failure. Family source grammars and mutations remain separate and fail closed; ZenUML never routes through the native Sequence adapter. | `session-workspace.tsx`, `zenuml-runtime.ts`, `diagram-capabilities.ts`, `diagram-preview.ts`, `diagram-canvas.tsx`, `zenuml-mutations.ts`, `svg-hit-map.ts` | `zenuml-runtime.test.ts`, `diagram-capabilities.test.ts`, `diagram-preview.test.ts`, `zenuml-mutations.test.ts`, `pnpm test:e2e-sequence`, `pnpm test:e2e-workspace-ux` |
...
| Per-diagram overlay scene | Durable/session, browser-only | Yjs `overlays`; server admission and persistence | Scene/object selection, active tool, drafts, resize handles, and camera stay local. Diagram deletion removes its scene and both private histories in one persistence batch. |
