...
| Canvas/editor presence | Ephemeral collaboration | Yjs awareness with per-socket client-id ownership | Editor cursor, active-diagram canvas cursor/selection, a bounded world-coordinate laser sample/sequence, and one advisory active node editor id travel only in awareness. Cursor and laser coordinates are quantized and coalesced client-side; selection/identity/removal updates are immediate. The server caps raw awareness state size, bounds canvas fields, filters stale/idempotent echoes, and rejects foreign advances. Awareness is not an authorization system, draft transport, lock, or browser UI store. |
