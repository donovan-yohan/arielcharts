...
async function waitForTransformChange(page: Page, layer: Locator, previous: string | null): Promise<string> {
...
async function validateSequenceCanvas() {
  ...
  try {
    ...
    const afterZoomTransform = await waitForTransformChange(page, transformedLayer, baselineTransform);
    await page.locator('button[aria-label="Fit diagram"]').click();
    const afterFitTransform = await waitForTransformChange(page, transformedLayer, afterZoomTransform);
    ...
    const afterPanTransform = await waitForTransformChange(page, transformedLayer, afterFitTransform);
    ...
    const zoomChangedTransform = baselineTransform !== afterZoomTransform;
    const fitChangedTransform = afterZoomTransform !== afterFitTransform;
    const panChangedTransform = afterFitTransform !== afterPanTransform;
    ...
    console.log(`generic Fit transform changed=${fitChangedTransform}`);
    console.log(`generic Fit bounds=${genericFitBounds.fits} transform=${afterFitTransform} viewBox=${genericFitBounds.viewBox} style=${genericFitBounds.svgStyle} svg=${JSON.stringify(genericFitBounds.svg)} canvas=${JSON.stringify(genericFitBounds.canvas)}`);
