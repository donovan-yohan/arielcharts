...
| Flowchart mutations | Mermaid AST -> mutation -> minimal Y.Text diff for representable nodes/edges; nested subgraph renames use one declaration-only source rewrite because AST rendering flattens nesting | `apps/web/src/lib/diagram-mutations.ts`, `apps/web/src/lib/diagram-subgraphs.ts` | `diagram-mutations.test.ts`, `diagram-subgraphs.test.ts`, `diagram-flow-identity.test.ts` |
...
| Canvas/editor presence | Ephemeral collaboration | Yjs awareness with per-socket client-id ownership | Editor cursor and active-diagram canvas cursor/selection travel only in awareness. Canvas cursor coordinates are quantized and coalesced client-side; canvas selection/identity/removal updates are immediate. The server caps raw awareness state size, bounds canvas fields, filters stale/idempotent echoes, and rejects foreign advances. Awareness is not an authorization system or browser UI store. |
