...
export function setBlockColumns(source: string, columns: number): string { requireBlock(source); const next = normalizeColumns(columns); const lines = splitLines(source); const headerIndex = statementIndex(lines); let depth = 0; for (let index = headerIndex + 1; index < lines.length; index += 1) { const line = lines[index]!; if (!line.text.trim() || ignorable(line.text)) continue; if (BLOCK.test(line.text)) { depth += 1; continue; } if (/^\s*end\s*$/i.test(line.text)) { depth -= 1; continue; } if (depth === 0 && COLUMNS.test(line.text)) return replace(source, line, `${indent(line)}columns ${next}`); } const header = lines[headerIndex]; if (!header) throw new Error('Block header no longer exists.'); return `${source.slice(0, header.end)}${lineEnding(source)}  columns ${next}${source.slice(header.end)}`; }
...
function parseBlock(source: string): Parsed | null {
  ...
  const nodes: NodeRecord[] = []; const composites: CompositeRecord[] = []; const links: LinkRecord[] = []; const stack: { columns?: number; columnsLine?: Line; id: string; line: Line; span: number }[] = []; let rootColumns: number | undefined;
  for (let index = headerIndex + 1; index < lines.length; index += 1) { const line = lines[index]!; if (!line.text.trim() || ignorable(line.text)) continue;
    if (/^\s*end\s*$/i.test(line.text)) { const open = stack.pop(); if (!open) return null; composites.push({ id: open.id, span: open.span, ...(open.columns ? { columns: open.columns } : {}), ...(open.columnsLine ? { columnsLine: open.columnsLine } : {}), line: open.line, close: line }); continue; }
