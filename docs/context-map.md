...
| Per-diagram overlay scene | Durable/local or online session | Yjs `overlays`; IndexedDB locally, server admission and persistence online | Browser owns selection, active tool, drafts, resize handles, and camera. MCP can list/read bounded current object records and make revision-checked object operations only after promotion; it never replaces a scene, writes awareness, or changes Mermaid source/layout. Diagram deletion removes its scene and both private histories in one online persistence batch. |
