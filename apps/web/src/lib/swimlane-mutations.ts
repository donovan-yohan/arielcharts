...
/** Changes lane membership by moving only the declaration; handoff identities keep their endpoint ids. */
export function moveSwimlaneNode(source: string, id: string, laneId: string): string { const parsed = requireSwimlane(source); const node = findNode(parsed, id); const targetId = normalizeId(laneId, 'Swimlane'); if (node.laneId === targetId) return source; const target = findLane(parsed, targetId); const without = deleteLines(source, [node.line]); const after = requireSwimlane(without); const afterTarget = findLane(after, target.id); return `${without.slice(0, afterTarget.close.start)}${indent(afterTarget.line)}  ${node.id}[${node.label}]${lineEnding(without)}${without.slice(afterTarget.close.start)}`; }
