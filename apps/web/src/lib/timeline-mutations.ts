...
export function addTimelinePeriod(source: string, period: TimelinePeriod): string { const next = normalizePeriod(period); if (!source.trim()) return `timeline LR\n  ${next.label}`; const parsed = requireTimeline(source); assertPeriod(parsed, next); const section = next.section ? findSection(parsed, next.section) : null; const following = section ? parsed.sections.find((candidate) => candidate.line.start > section.line.start) : parsed.sections[0]; const offset = following?.line.start ?? source.length; const prefix = source.slice(0, offset); const ending = lineEnding(source); return `${prefix}${prefix && !/(?:\r\n|\n|\r)$/.test(prefix) ? ending : ''}${section ? indent(section.line) : '  '}${next.label}${ending}${source.slice(offset)}`; }
...
export function editTimelineEvent(source: string, identity: TimelineEventIdentity, patch: Partial<TimelineEvent>): string { const parsed = requireTimeline(source); const current = parsed.events[resolveTimelineEventIndex(parsed.events, identity)]; if (!current) throw stale(); const next = normalizeEvent({ ...current, ...patch }); if (next.period !== current.period) { const period = findPeriod(parsed, current.period); const without = current.line.start === period.line.start ? replaceLine(source, current.line, `${indent(period.line)}${period.label}`) : deleteLines(source, [current.line]); return addTimelineEvent(without, next); } if (next.section !== current.section) throw new Error('Timeline events must remain in their period section.'); const period = findPeriod(parsed, current.period); return current.line.start === period.line.start ? replaceLine(source, current.line, replaceEventText(current.line, next.text)) : replaceLine(source, current.line, `${indent(current.line)}: ${next.text}`); }
